...
def _can_view_all(role):
    """Admin and HR can see all employees' attendance. Payroll Officer gets read-only view."""
    return role in [ADMIN, HR, PAYROLL]
...
def get_attendance(current_user):
    ...
    # Admin, HR, Payroll Officer can view any employee's attendance.
    # Employees can only view their own.
    if not _can_view_all(role) and int(employee_id) != current_user['id']:
        return jsonify({'error': 'Unauthorized — you can only view your own attendance'}), 403
...
def mark_attendance(current_user):
    """Admin and HR can mark/edit attendance for any employee."""
    data        = request.json
    role        = current_user['role']
    employee_id = data.get('employeeId', current_user['id'])

    # Employees can only mark their own; HR/Admin can mark any
    if role not in [ADMIN, HR] and int(employee_id) != current_user['id']:
...
def check_out(current_user):
    ...
    if not record:
        return jsonify({'error': 'No check-in found. Please check in first.'}), 400
...
def auto_checkout(current_user):
    ...
    try:
        ...
        work_hours = round((co - ci).seconds / 3600, 2) if co > ci else 0
    except Exception:
        work_hours = 0
...
def get_today_attendance(current_user):
    ...
    if _can_view_all(role):
        # Admin, HR, Payroll Officer: all employees' attendance
        records = conn.execute('''
            SELECT u.id as user_id, u.first_name, u.last_name, u.login_id, u.department,
